...
Write a query calculating the amount of error (i.e.: actual-miscalculated average monthly salaries), and
round it up to the next integer.
*/
SELECT CEIL((SELECT AVG(SALARY) FROM EMPLOYEES) -
(SELECT AVG(CAST(REPLACE(CONV(SALARY,10,10),'0','') as DECIMAL(10,6))) FROM EMPLOYEES));